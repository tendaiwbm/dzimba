...
def pipeline(source):
    ...
    filteredListings = apply_filters(currentListings,source["filters"],source["model"])
    knownListings = get_create_local_copy(source["path"],source["localFileName"],filteredListings)
    newListings = extract_new_listings(knownListings,filteredListings,source["model"].id_)
